...
def lenet(image_batch):
    h_conv1 = layers.Conv2D(filters=96, kernel_size=11, strides=4, padding='valid', activation='relu', use_bias=True)(image_batch)
    h_pool1 = layers.MaxPooling2D(pool_size=3, strides=2)(h_conv1)
    h_conv2 = layers.Conv2D(filters=256, kernel_size=5, padding='same', activation='relu', use_bias=True)(h_pool1)
    h_pool2 = layers.MaxPooling2D(pool_size=3, strides=2)(h_conv2)
    h_conv3 = layers.Conv2D(filters=384, kernel_size=3, padding='same', activation='relu', use_bias=True)(h_pool2)
    h_conv4 = layers.Conv2D(filters=384, kernel_size=3, padding='same', activation='relu', use_bias=True)(h_conv3)
    h_conv5 = layers.Conv2D(filters=256, kernel_size=3, padding='same', activation='relu', use_bias=True)(h_conv4)
